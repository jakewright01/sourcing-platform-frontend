    ...
    if (listingId) {
      const checkAdminAndFetchListing = async () => {
        ...
        try {
          ...
          setFormData({ item_name: data.item_name || '', item_description: data.item_description || '', price: data.price || 0, condition: data.condition || 'New' });
        } catch (err) {

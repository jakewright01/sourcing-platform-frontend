    ...
    <main className="flex min-h-screen flex-col items-center p-8 sm:p-12 md:p-24 bg-zinc-900 text-white">
      <div className="w-full max-w-6xl">
        <div className="flex justify-between items-center mb-10">
          <h1 className="text-4xl font-bold tracking-tighter">Admin: All Listings</h1>
          <Link href="/admin/add-listing" className="bg-white hover:bg-zinc-200 text-black font-semibold py-2 px-4 rounded-lg transition-colors">
            + Add New Listing
          </Link>
        </div>
        ...
        <div className="bg-black border border-zinc-800 rounded-2xl shadow-lg overflow-hidden">
          <table className="min-w-full">
            <thead className="bg-zinc-900">
              <tr>
                <th scope="col" className="py-3 px-6 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">Item Name</th>
                <th scope="col" className="py-3 px-6 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">Price</th>
                <th scope="col" className="py-3 px-6 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">Condition</th>
                <th scope="col" className="relative py-3 px-6"><span className="sr-only">Actions</span></th>
              </tr>
            </thead>
            <tbody className="bg-black divide-y divide-zinc-800">
              {listings.map((listing) => (
                <tr key={listing.id} className="hover:bg-zinc-900">
                  <td className="py-4 px-6 whitespace-nowrap font-medium text-white">{listing.item_name}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-zinc-300">£{(listing.price || 0).toFixed(2)}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-zinc-300">{listing.condition}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-right text-sm font-medium space-x-3">
